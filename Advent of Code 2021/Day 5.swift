...
func day5() {
    struct VentLine {
        ...
        /// Only inits if line is vertical or horizontal
        init?(_ string: String) {
            let startEnd = string.components(separatedBy: " -> ")
            
            guard let start = startEnd.first, let end = startEnd.last else { return nil }
            
            let startPoints = start.components(separatedBy: ",")
            let endPoints = end.components(separatedBy: ",")
            
            guard
                let startX = startPoints.first,
    ...
    struct VentGrid {
        ...
        init(_ string: String) {
            let coords = string.components(separatedBy: .newlines)
            ventLines = coords.compactMap { VentLine($0) }
        }
        
        var overlappingVents: Int? {
            var dict = [CGPoint: Int]()
            
            for line in ventLines {
                for point in line.points {
                    if let count = dict[point] {
                        dict[point] = count + 1
                    } else {
                        dict[point] = 1
                    }
            ...
            let atLeast2Vents = dict.values.filter { $0 >= 2 }
    ...
    print(puzzleGrid.overlappingVents) // 17604 (correct)
}
