...
func day18() {
    ...
    for (firstIndex, firstAny) in puzzleInput.enumerated() {
        for (secondIndex, secondAny) in puzzleInput.enumerated() {
            guard firstIndex != secondIndex else { continue }
            
            print("Testing \(firstIndex) - \(secondIndex)")
    ...
    print(largestMagnitude) // 4673 (correct)
}
