...
//  Advent of Code 2021
//
//  Created by James Coleman on 04/12/2021.
//
...
func day4() {
    ...
    let boardStrings = puzzleBoards.components(separatedBy: "\n\n")

    let boards = boardStrings.map { BingoBoard($0) }

    var winningBoards = [(BingoBoard, Int)]()
    
    for number in puzzleInput {
        for board in boards {
            guard board.won == false else { continue }
            
            board.marked(number)
            
            if let _ = board.winningNumbers {
                board.won = true
                
                winningBoards += [(board, number)]
            }
        }
    }
    
    if let lastWinningBoard = winningBoards.last {
        let sum = lastWinningBoard.0.sumOfUnmarkedNumbers
        
        let score = sum * lastWinningBoard.1
        
        print(score) // 13158 (correct)
    }
}
